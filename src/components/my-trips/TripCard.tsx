...
import { TripDetailsModal } from "@/components/my-trips/TripDetailsModal";
import { EditTripModal } from "@/components/my-trips/EditTripModal";
import { TripSummaryModal } from "@/components/my-trips/TripSummaryModal";
...
type ModalType = "details" | "edit" | "summary" | null;
...
// Map action label → modal type
function modalForAction(label: string): ModalType {
  if (label === "View Details") return "details";
  if (label === "Edit Trip")    return "edit";
  if (label === "View Summary") return "summary";
  return null;
}
...
export function TripCard({ trip, index }: TripCardProps) {
  ...
  return (
    <>
          ...
          <div>
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <p className={cn("mt-1 text-3xl font-bold", trip.status === "cancelled" ? "text-[#B84040]" : "text-[#1F261F]")}>
                  {trip.budgetValue}
                </p>
              </div>
          ...
          <div className="flex flex-col justify-between gap-4">
            <div className="space-y-2">
              {trip.actions.map((action) => {
                ...
                const modal = modalForAction(action.label);
                return (
                  <button
                    key={action.label}
                    type="button"
                    onClick={() => modal && setOpenModal(modal)}
                    className="flex h-12 w-full items-center justify-center gap-2 rounded-xl border border-[#D9D0C2] bg-[#FCF9F4] px-3 text-sm font-semibold text-[#2D2A24] transition-all duration-300 hover:-translate-y-0.5 hover:bg-white hover:border-[#2F4F3E]/30 hover:text-[#2F4F3E]"
                  >
      ...
      {/* Modals */}
      {openModal === "details" && (
        <TripDetailsModal trip={trip} onClose={() => setOpenModal(null)} />
      )}
      {openModal === "edit" && (
        <EditTripModal trip={trip} onClose={() => setOpenModal(null)} />
      )}
      {openModal === "summary" && (
        <TripSummaryModal trip={trip} onClose={() => setOpenModal(null)} />
      )}
    </>
  );
}
