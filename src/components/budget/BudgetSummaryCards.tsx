...
import { summaryStats } from "@/data/budget";
...
const cards: StatCard[] = [
  {
    label: "Total Budget",
    amount: `$${summaryStats.totalBudget.toLocaleString()}`,
    sub: "Planned Budget",
  ...
  {
    label: "Total Spent",
    amount: `$${summaryStats.totalSpent.toLocaleString()}`,
    sub: `${Math.round((summaryStats.totalSpent / summaryStats.totalBudget) * 100)}% of budget`,
    icon: <TrendingUp className="h-5 w-5 text-[#E87565]" strokeWidth={1.8} />,
    iconBg: "bg-[#FDECEA]"
  },
  {
    label: "Remaining",
    amount: `$${summaryStats.remaining.toLocaleString()}`,
    sub: `${Math.round((summaryStats.remaining / summaryStats.totalBudget) * 100)}% left to spend`,
    icon: <TrendingDown className="h-5 w-5 text-[#3F7EA7]" strokeWidth={1.8} />,
    iconBg: "bg-[#E6F1F8]"
  },
  {
    label: "Avg. Cost per Day",
    amount: `$${summaryStats.avgCostPerDay}`,
    sub: `Based on ${summaryStats.tripDays} days`,
  ...
  return (
    <div className="grid grid-cols-2 gap-4 xl:grid-cols-4">
      {cards.map((card) => (
        ...
        >
          <div className={`inline-flex rounded-xl p-2.5 ${card.iconBg}`}>
            {card.icon}
          </div>
          <p className="mt-3 text-sm font-medium text-[#7F7A70]">{card.label}</p>
          <p className="mt-1 font-serif text-3xl font-bold text-[#1F261F]">
            {card.amount}
          </p>
